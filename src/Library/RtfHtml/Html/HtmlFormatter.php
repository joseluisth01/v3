...
class HtmlFormatter
{
    ...
    public function format2Txt(Document $document)
    {
        $this->output = '';
    ...
    protected function loadFont(\App\Library\RtfHtml\Group $fontGroup)
    {
        $fontNumber = 0;
        $font = new Font();
        ...
        foreach ($fontGroup->children as $child) {
            // Control word
            if ($child instanceof \App\Library\RtfHtml\ControlWord) {
                switch ($child->word) {
                    case 'f':
            ...
            if ($child instanceof \App\Library\RtfHtml\Text) {
                // Store font name (except ; delimiter at end)
                $font->name = substr($child->text, 0, -1);
            }

            /*
            elseif ($child instanceof \App\Library\RtfHtml\Group) {
                    // possible subgroups:
                    ...
                    // '{\*' \panose <data> '}'
                    continue;
                  } elseif ($child instanceof \App\Library\RtfHtml\ControlSymbol) {
    ...
    protected function extractFontTable($fontTblGrp)
    {
        ...
        foreach ($fontTblGrp->children as $child) {
            ...
            if (!($child instanceof \App\Library\RtfHtml\Group)) {
                continue;
            }
    ...
    protected function extractColorTable($colorTblGrp)
    {
        ...
        for ($i = 1; $i < $c; ++$i) { // Iterate through colors
            if ($colorTblGrp[$i] instanceof \App\Library\RtfHtml\ControlWord) {
                ...
                $i += 2;
            } elseif ($colorTblGrp[$i] instanceof \App\Library\RtfHtml\Text) {
    ...
    protected function extractImage($pictGrp)
    {
        ...
        foreach ($pictGrp as $child) {
            ...
            } elseif ($child instanceof \App\Library\RtfHtml\Text) { // store Data
    ...
    {
        if (!$dest[1] instanceof \App\Library\RtfHtml\ControlWord) {
            return;
        }
    ...
    {
        if ($entry instanceof \App\Library\RtfHtml\Group) {
            $this->processGroup($entry);
        } elseif ($entry instanceof \App\Library\RtfHtml\ControlWord) {
            $this->formatControlWord($entry);
        } elseif ($entry instanceof \App\Library\RtfHtml\ControlSymbol) {
            $this->formatControlSymbol($entry);
        } elseif ($entry instanceof \App\Library\RtfHtml\Text) {
    ...
    protected function decodeUnicode($code, $srcEnc = 'UTF-8')
    {
        ...
        if ($this->encoding == 'HTML-ENTITIES') {
            return $utf8 ? "&#{$this->ordUTF8($utf8)};" : "&#{$code};";
